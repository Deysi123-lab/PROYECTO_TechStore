...
export class ClienteCheckoutComponent implements OnInit, OnDestroy {
  ...
  private validarPago(): boolean {
    ...
    if (metodo === 'YAPE') {
      if (f.yapeCelular.value.replace(/\D/g, '').length < 9) {
        this.error.set('Ingresa el celular asociado a Yape.');
        return false;
      }
    ...
    this.pedidoService
      ...
      .subscribe({
        next: (pedido) => {
          ...
          this.toast.success(`¡Pedido ${pedido.id} confirmado!`);
          void this.router.navigate(['/cliente/pedidos'], {
            state: {
              mensaje: `Compra exitosa. Pedido ${pedido.id} — pago con ${metodo} registrado.`,
            },
          });
        },
        error: (err) => {
